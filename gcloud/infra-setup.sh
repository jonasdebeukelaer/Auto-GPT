...
MINIO_PASSWORD_SECRET_NAME="crypto-gpt-minio-password"
...
# allow compute to act as user account ??
gcloud projects add-iam-policy-binding "$PROJECT_ID" \
    --member "serviceAccount:$PROJECT_NUMBER@cloudbuild.gserviceaccount.com" \
...
# allow google cloud run access to minio secrets
gcloud secrets add-iam-policy-binding ${MINIO_PASSWORD_SECRET_NAME} \
  --member "serviceAccount:${SERVICE_ACCOUNT}" \
  --role roles/secretmanager.secretAccessor
